...
import { 
  ...
  Award, CreditCard, Lock, Plus, Trash2, Grid, ChevronLeft, ChevronRight, FileSpreadsheet
} from "lucide-react";
import { QRCodeSVG } from "qrcode.react";
import { ArtistProfile } from "../types.js";
import { useLanguage } from "../i18n/LanguageContext.js";
import { 
  ExhibitionCartelItem, 
  CartelDensity, 
  CARTEL_DENSITIES, 
  generateA4CartelsHtml, 
  downloadA4CartelsHtmlFile, 
  printA4CartelsViaIframe, 
  openA4CartelsInNewTab 
} from "../utils/cartelExport.js";
...
export default function QrSalesCartelModal({
  ...
  // =========================================================================
  // A4 CARTEL SHEETS & MULTI-ARTWORK EXHIBITION LOGIC
  // =========================================================================
  const [cartelDensity, setCartelDensity] = useState<CartelDensity>("standard");
  const [exhibitionTitle, setExhibitionTitle] = useState<string>("Exposition Collective d'Atelier");
  const [isAddingArtwork, setIsAddingArtwork] = useState<boolean>(false);
  const [cartelActionNotice, setCartelActionNotice] = useState<string | null>(null);

  // New artwork form fields for rapid addition to exhibition
  const [newArtTitle, setNewArtTitle] = useState<string>("");
  const [newArtArtist, setNewArtArtist] = useState<string>(profile.name || "Artiste");
  const [newArtMedium, setNewArtMedium] = useState<string>("Tirage photographique d'art");
  const [newArtYear, setNewArtYear] = useState<string>(new Date().getFullYear().toString());
  const [newArtDimensions, setNewArtDimensions] = useState<string>("60 × 40 cm");
  const [newArtPrice, setNewArtPrice] = useState<string>("850 €");
  const [newArtType, setNewArtType] = useState<"peinture" | "photo" | "sculpture" | "dessin" | "autre">("photo");

  // Initial exhibition artworks list
  const [exhibitionArtworks, setExhibitionArtworks] = useState<ExhibitionCartelItem[]>(() => {
    return [
      {
        id: "art-active",
        title: activeArtworkTitle || "Sans Titre (Étude d'Atelier)",
        artist: profile.name || "Artiste d'Atelier",
        medium: activeArtworkMedium || "Huile et pigments sur toile",
        year: activeArtworkYear || "2026",
        dimensions: activeArtworkDimensions || "100 × 80 cm",
        price: "2 800 €",
        status: "available",
        type: "peinture",
        qrUrl: typeof window !== 'undefined'
          ? `${window.location.origin}/#visiteur-oeuvre-${encodeURIComponent((activeArtworkTitle || 'oeuvre').replace(/\s+/g, '-').toLowerCase())}`
          : `https://art-atelier.app/#visiteur`,
        copies: 1,
        enabled: true
      }
  ...
  // Auto-sync custom artworks (paintings & photos) uploaded by the user
  useEffect(() => {
    if (customArtworks && customArtworks.length > 0) {
      setExhibitionArtworks(prev => {
        const existingIds = new Set(prev.map(p => p.id));
        const toAdd: ExhibitionCartelItem[] = [];
        customArtworks.forEach((art, idx) => {
          const id = `custom-${art.id || idx}`;
          if (!existingIds.has(id)) {
            const isPhoto = (art.medium || "").toLowerCase().includes("photo") || (art.title || "").toLowerCase().includes("photo");
            toAdd.push({
              id,
              title: art.title || `Œuvre d'Atelier #${idx + 1}`,
              artist: art.artist || profile.name || "Artiste",
              medium: art.medium || (isPhoto ? "Tirage d'art argentique" : "Huile et pigments"),
              year: art.year || "2026",
              dimensions: art.dimensions || "60 × 40 cm",
              price: "1 200 €",
              status: "available",
              type: isPhoto ? "photo" : "peinture",
              qrUrl: typeof window !== 'undefined'
                ? `${window.location.origin}/#visiteur-oeuvre-${encodeURIComponent((art.title || 'oeuvre').replace(/\s+/g, '-').toLowerCase())}`
                : `https://art-atelier.app/#visiteur`,
              copies: 1,
              enabled: true
            });
  ...
  }, [customArtworks, profile.name]);

  // Keep the active artwork cartel synchronized with tab 1 inputs
  useEffect(() => {
    setExhibitionArtworks(prev => {
      const idx = prev.findIndex(item => item.id === "art-active");
      ...
      updated[idx] = {
        ...
        price: isPriceOnDemand ? "Prix sur demande" : artworkPrice,
        status: artworkStatus,
        qrUrl: qrPayload
      };
      return updated;
    });
  }, [artworkTitle, artistName, artworkMedium, artworkYear, artworkDimensions, artworkPrice, isPriceOnDemand, artworkStatus, qrPayload]);

  // Flatten cartels based on enabled status and number of copies
  const flattenedCartels = useMemo(() => {
    const list: ExhibitionCartelItem[] = [];
    exhibitionArtworks.forEach(item => {
      if (item.enabled && item.copies > 0) {
    ...
    return list.length > 0 ? list : [
      {
        id: "fallback-active",
        title: artworkTitle || "Sans Titre",
        artist: artistName || "Artiste",
        medium: artworkMedium || "Technique Mixte",
        year: artworkYear || "2026",
        dimensions: artworkDimensions || "100 × 80 cm",
        price: artworkPrice || "2 800 €",
        status: "available",
        type: "peinture",
        qrUrl: qrPayload,
        copies: 1,
  ...
  }, [exhibitionArtworks, artworkTitle, artistName, artworkMedium, artworkYear, artworkDimensions, artworkPrice, qrPayload]);

  const densityConfig = CARTEL_DENSITIES[cartelDensity] || CARTEL_DENSITIES.standard;
  const perPage = densityConfig.cartelsPerPage;
  const totalPages = Math.max(1, Math.ceil(flattenedCartels.length / perPage));

  const pagedCartels = useMemo(() => {
    const pages: ExhibitionCartelItem[][] = [];
    for (let i = 0; i < flattenedCartels.length; i += perPage) {
      pages.push(flattenedCartels.slice(i, i + perPage));
    }
    return pages.length > 0 ? pages : [[]];
  }, [flattenedCartels, perPage]);

  // Helper to extract rendered SVG strings from the DOM
  const getRenderedExportCartels = () => {
    const svgs = printRef.current ? Array.from(printRef.current.querySelectorAll('svg.cartel-qr-code')) : [];
    return flattenedCartels.map((item, idx) => {
      const svgEl = svgs[idx] as Element | undefined;
      return {
        item,
        qrSvgHtml: svgEl && svgEl.outerHTML
          ? svgEl.outerHTML
          : `<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><rect width="100" height="100" fill="#ffffff"/><rect x="15" y="15" width="70" height="70" fill="#111111"/></svg>`
      };
    });
  };

  const handleSaveA4File = () => {
    const cartels = getRenderedExportCartels();
    const html = generateA4CartelsHtml({
      galleryName,
      exhibitionTitle,
      density: cartelDensity,
      cartels
    });
    const filename = `Planche_Cartels_A4_${exhibitionTitle.replace(/[^a-zA-Z0-9_-]/g, "_") || "Exposition"}`;
    downloadA4CartelsHtmlFile(filename, html);
    setCartelActionNotice("Fichier A4 (.html) enregistré avec succès ! Ouvrez-le à tout moment pour l'imprimer ou l'exporter en PDF.");
    setTimeout(() => setCartelActionNotice(null), 5000);
  };
  ...
  const handlePrintA4 = () => {
    ...
    printA4CartelsViaIframe(html);
  };

  const handleOpenA4InNewTab = () => {
    const cartels = getRenderedExportCartels();
    const html = generateA4CartelsHtml({
      galleryName,
      exhibitionTitle,
  ...
  const handleCopyA4Html = () => {
    ...
    setCartelActionNotice("Code HTML de la planche A4 copié dans le presse-papier !");
    setTimeout(() => setCartelActionNotice(null), 4000);
  };

  const handleFillCurrentArtwork = () => {
    setExhibitionArtworks(prev => {
      return prev.map(art => {
        if (art.id === "art-active") {
          return { ...art, enabled: true, copies: perPage };
        }
        return { ...art, enabled: false };
      });
    });
    setCartelActionNotice(`Feuille A4 remplie avec l'œuvre courante (${perPage} cartels).`);
    setTimeout(() => setCartelActionNotice(null), 4000);
  };

  const handleResetOneEach = () => {
    setExhibitionArtworks(prev => prev.map(art => ({ ...art, enabled: true, copies: 1 })));
    setCartelActionNotice("Copies réinitialisées : 1 exemplaire par œuvre.");
    setTimeout(() => setCartelActionNotice(null), 3000);
  };

  const handleToggleAllArtworks = () => {
    const anyEnabled = exhibitionArtworks.some(a => a.enabled);
    setExhibitionArtworks(prev => prev.map(art => ({ ...art, enabled: !anyEnabled })));
  };

  const handleLoadSampleExhibition = () => {
    const sampleArtworks: ExhibitionCartelItem[] = [
      {
        id: "sample-p1",
        title: "Soleil Noir & Matières",
        artist: profile.name || "Artiste",
        medium: "Huile et pigments naturels sur lin",
        year: "2026",
        dimensions: "120 × 90 cm",
        price: "3 200 €",
        status: "available",
        type: "peinture",
        qrUrl: typeof window !== 'undefined' ? `${window.location.origin}/#visiteur-p1` : "https://art-atelier.app/#p1",
        copies: 1,
        enabled: true
      },
      {
        id: "sample-ph1",
        title: "Lumière d'Atelier #04",
        artist: profile.name || "Artiste",
        medium: "Tirage argentique baryté sous passe-partout",
        year: "2026",
        dimensions: "60 × 40 cm",
        price: "950 €",
        status: "available",
        type: "photo",
        qrUrl: typeof window !== 'undefined' ? `${window.location.origin}/#visiteur-ph1` : "https://art-atelier.app/#ph1",
        copies: 1,
        enabled: true
      },
      {
        id: "sample-p2",
        title: "Nocturne Minéral",
        artist: profile.name || "Artiste",
        medium: "Acrylique et poudre de marbre sur châssis",
        year: "2025",
        dimensions: "100 × 100 cm",
        price: "2 800 €",
        status: "reserved",
        type: "peinture",
        qrUrl: typeof window !== 'undefined' ? `${window.location.origin}/#visiteur-p2` : "https://art-atelier.app/#p2",
        copies: 1,
        enabled: true
      },
      {
        id: "sample-ph2",
        title: "Brume Portuaire",
        artist: profile.name || "Artiste",
        medium: "Tirage pigmentaire UltraChrome Hahnemühle",
        year: "2026",
        dimensions: "50 × 75 cm",
        price: "850 €",
        status: "available",
        type: "photo",
        qrUrl: typeof window !== 'undefined' ? `${window.location.origin}/#visiteur-ph2` : "https://art-atelier.app/#ph2",
        copies: 1,
        enabled: true
      },
      {
        id: "sample-p3",
        title: "Symphonie en Ocre et Cendres",
        artist: profile.name || "Artiste",
        medium: "Technique mixte et collage sur bois",
        year: "2026",
        dimensions: "80 × 60 cm",
        price: "1 950 €",
        status: "available",
        type: "peinture",
        qrUrl: typeof window !== 'undefined' ? `${window.location.origin}/#visiteur-p3` : "https://art-atelier.app/#p3",
        copies: 1,
        enabled: true
      },
      {
        id: "sample-ph3",
        title: "Reflet Géométrique",
        artist: profile.name || "Artiste",
        medium: "Photographie numérique, tirage C-Print contrecollé Dibond",
        year: "2025",
        dimensions: "70 × 105 cm",
        price: "1 400 €",
        status: "sold",
        type: "photo",
        qrUrl: typeof window !== 'undefined' ? `${window.location.origin}/#visiteur-ph3` : "https://art-atelier.app/#ph3",
        copies: 1,
        enabled: true
      },
      {
        id: "sample-p4",
        title: "Élévation Tellurique",
        artist: profile.name || "Artiste",
        medium: "Encre et détrempe sur papier marouflé",
        year: "2026",
        dimensions: "65 × 50 cm",
        price: "1 100 €",
        status: "available",
        type: "peinture",
        qrUrl: typeof window !== 'undefined' ? `${window.location.origin}/#visiteur-p4` : "https://art-atelier.app/#p4",
        copies: 1,
        enabled: true
      },
      {
        id: "sample-sc1",
        title: "Élan de Bronze",
        artist: profile.name || "Artiste",
        medium: "Bronze patiné à chaud, socle granit noir (Édition 1/8)",
        year: "2026",
        dimensions: "45 × 25 × 20 cm",
        price: "4 600 €",
        status: "available",
        type: "sculpture",
        qrUrl: typeof window !== 'undefined' ? `${window.location.origin}/#visiteur-sc1` : "https://art-atelier.app/#sc1",
        copies: 1,
        enabled: true
      }
    ];
    ...
    setExhibitionArtworks(prev => {
      ...
      const toAdd = sampleArtworks.filter(s => !existingTitles.has(s.title.toLowerCase()));
      return [...prev, ...toAdd];
    });

    setCartelActionNotice("Exposition enrichie avec 8 œuvres variées (peintures, photos d'art et sculptures) !");
    setTimeout(() => setCartelActionNotice(null), 4000);
  };
  ...
  const handleAddNewArtwork = (e?: React.FormEvent) => {
    ...
    if (!newArtTitle.trim()) {
      alert("Veuillez saisir au moins un titre pour l'œuvre.");
      return;
    }

    const newItem: ExhibitionCartelItem = {
      id: `art-custom-${Date.now()}`,
      title: newArtTitle.trim(),
      artist: newArtArtist.trim() || profile.name || "Artiste",
      medium: newArtMedium.trim() || "Technique Mixte",
      year: newArtYear.trim() || new Date().getFullYear().toString(),
      dimensions: newArtDimensions.trim() || "Format libre",
      price: newArtPrice.trim() || "Prix sur demande",
      status: "available",
      type: newArtType,
      qrUrl: typeof window !== 'undefined'
        ? `${window.location.origin}/#visiteur-oeuvre-${encodeURIComponent(newArtTitle.trim().replace(/\s+/g, '-').toLowerCase())}`
        : `https://art-atelier.app/#visiteur`,
      copies: 1,
      enabled: true
    };

    setExhibitionArtworks(prev => [newItem, ...prev]);
    setNewArtTitle("");
    setIsAddingArtwork(false);
    setCartelActionNotice(`Nouvelle œuvre "${newItem.title}" ajoutée à l'exposition !`);
    setTimeout(() => setCartelActionNotice(null), 4000);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 md:p-6 bg-black/85 backdrop-blur-md animate-fadeIn">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
          ...
          {/* TAB 4: PLANCHE D'IMPRESSION & EXPORT A4 (CARTELS SANS CONTOUR DE DÉCOUPE) */}
          {activeTab === "print_cartels" && (
            <div className="space-y-6 animate-fadeIn">
              
              {/* Notification Banner */}
              {cartelActionNotice && (
                <div className="p-3.5 bg-emerald-500/15 border border-emerald-500/40 text-emerald-300 text-xs flex items-center justify-between gap-3 rounded-none animate-fadeIn">
                  ...
                  <button 
                    type="button" 
                    onClick={() => setCartelActionNotice(null)}
                    className="text-emerald-400 hover:text-white text-xs uppercase font-mono cursor-pointer"
                  >
                    Fermer
                  </button>
                </div>
              ...
              {/* Main Actions Bar */}
              <div className={`p-4 sm:p-5 border flex flex-col md:flex-row items-start md:items-center justify-between gap-4 ${
                isDark ? "bg-black/50 border-white/10" : "bg-stone-50 border-stone-300"
              }`}>
                <div>
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 text-[9px] font-mono uppercase tracking-widest bg-[#c9a84c] text-black font-bold">
                      Format ISO A4
                    </span>
                    <span className="text-[10px] font-mono text-stone-400 uppercase tracking-wider">
                      210 × 297 mm • Sans contour de coupe
                    </span>
                  </div>
                  <h3 className="font-serif font-bold text-lg text-[#c9a84c] mt-1">
                    Planche d'Impression & Sauvegarde A4 des Cartels
                  </h3>
                  <p className="text-xs opacity-75 mt-0.5 max-w-2xl">
                    Conforme aux exigences d'exposition : cartels de 5 cm sur 3 à 4 cm, sans repères ni liserés de coupe pour éviter tout trait noir ou gris résiduel après massicotage. Prise en charge des expositions multi-œuvres (peintures & photographies).
                  </p>
                </div>

                {/* Primary Export & Print Actions */}
                <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                  <button
                    type="button"
                    onClick={handleSaveA4File}
                    className="flex-1 sm:flex-none py-2.5 px-4 bg-[#c9a84c] hover:bg-[#d8b85c] text-black font-sans font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-all cursor-pointer shadow-md"
                    title="Enregistrer le fichier A4 autonome (.html) pour impression ou archivage"
                  >
                    <FileDown className="w-4 h-4" /> Enregistrer le Fichier A4
                  </button>
                  <button
                    type="button"
                    onClick={handlePrintA4}
                    className="flex-1 sm:flex-none py-2.5 px-4 bg-stone-800 hover:bg-stone-700 text-white border border-stone-600 font-sans font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-all cursor-pointer shadow-sm"
                    title="Imprimer directement les feuilles A4 via l'imprimante système"
                  >
                    <Printer className="w-4 h-4" /> Imprimer en A4
                  </button>
                  <button
                    type="button"
                    onClick={handleOpenA4InNewTab}
                    className={`p-2.5 border text-xs transition-colors cursor-pointer ${
                      isDark ? "border-neutral-700 hover:bg-white/10 text-stone-300" : "border-stone-300 hover:bg-stone-200 text-stone-700"
                    }`}
                    title="Ouvrir la planche dans un nouvel onglet"
                  >
                  ...
                  <button
                    ...
                    title="Copier le code HTML de la planche A4"
                  >
              ...
              {/* Configuration & Artworks Grid */}
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                
                {/* Configuration Panel */}
                <div className={`p-4 border space-y-4 ${isDark ? "bg-black/30 border-white/10" : "bg-white border-stone-200"}`}>
                  <div className="flex items-center gap-2 pb-2 border-b border-white/10">
                    <Sliders className="w-4 h-4 text-[#c9a84c]" />
                    <h4 className="font-serif font-bold text-xs uppercase tracking-wider text-[#c9a84c]">
                      Format & Densité des Cartels
                    </h4>
                  </div>

                  <div>
                    <label className="block text-[11px] font-mono text-stone-400 mb-1.5 uppercase">
                      Taille normalisée sur A4
                    </label>
                    <div className="grid grid-cols-1 gap-2">
                      {(Object.keys(CARTEL_DENSITIES) as CartelDensity[]).map(key => {
                        const conf = CARTEL_DENSITIES[key];
                        const isSelected = cartelDensity === key;
                        return (
                          <button
                            key={key}
                            type="button"
                            onClick={() => setCartelDensity(key)}
                            className={`p-2.5 text-left border transition-all cursor-pointer ${
                              isSelected
                                ? "border-[#c9a84c] bg-[#c9a84c]/10 text-white"
                                : isDark
                                  ? "border-neutral-800 bg-neutral-900/50 text-stone-300 hover:border-neutral-700"
                                  : "border-stone-200 bg-stone-50 text-stone-700 hover:border-stone-400"
                            }`}
                          >
                            <div className="flex items-center justify-between">
                              <span className="text-xs font-bold font-sans">{conf.label}</span>
                              <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded-none font-bold ${
                                isSelected ? "bg-[#c9a84c] text-black" : "bg-stone-800 text-stone-400"
                              }`}>
                                {conf.cartelsPerPage} / page
                              </span>
                            </div>
                            <p className="text-[10px] text-stone-400 mt-1">{conf.description}</p>
                          </button>
                        );
                      })}
                    </div>
                  ...
                  <div className="space-y-2 pt-2 border-t border-white/10">
                    <div>
                      <label className="block text-[10px] font-mono text-stone-400 mb-1 uppercase">
                      ...
                      <input
                        ...
                        placeholder="Exposition d'Atelier..."
                        className={`w-full p-2 text-xs border ${
                          isDark ? "bg-neutral-900 border-neutral-700 text-white" : "bg-white border-stone-300 text-black"
                        }`}
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] font-mono text-stone-400 mb-1 uppercase">
                        Nom de la Galerie / Espace
                      </label>
                      <input
                        type="text"
                        value={galleryName}
                        onChange={(e) => setGalleryName(e.target.value)}
                        placeholder="Galerie d'Art..."
                        className={`w-full p-2 text-xs border ${
                          isDark ? "bg-neutral-900 border-neutral-700 text-white" : "bg-white border-stone-300 text-black"
                        }`}
                      />
                  ...
                  <div className="p-3 bg-amber-500/10 border border-amber-500/20 text-[11px] text-amber-200/90 leading-relaxed">
                    <p className="font-bold mb-1 flex items-center gap-1.5 text-amber-300">
                      <Info className="w-3.5 h-3.5" /> Découpe Pro & Zéro Liseré :
                    </p>
                    Les cartels sont espacés par du blanc pur. Coupez simplement au milieu des blancs avec un massicot ou cutter : aucun trait de contour n'apparaîtra sur les bords de vos cartels.
                  </div>
                </div>

                {/* Exhibition Artworks Manager */}
                <div className={`lg:col-span-2 p-4 border space-y-4 ${isDark ? "bg-black/30 border-white/10" : "bg-white border-stone-200"}`}>
                  <div className="flex flex-wrap items-center justify-between gap-2 pb-2 border-b border-white/10">
                    <div className="flex items-center gap-2">
                      <Layers className="w-4 h-4 text-[#c9a84c]" />
                      <h4 className="font-serif font-bold text-xs uppercase tracking-wider text-[#c9a84c]">
                        Œuvres de l'Exposition ({exhibitionArtworks.filter(a => a.enabled).length}/{exhibitionArtworks.length})
                      </h4>
                    </div>
                    <span className="text-[10px] font-mono text-stone-400">
                      {flattenedCartels.length} cartels au total • {totalPages} page{totalPages > 1 ? "s" : ""} A4
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
                    <button
                      type="button"
                      onClick={handleToggleAllArtworks}
                      className={`px-2.5 py-1 border text-[10px] font-mono uppercase cursor-pointer ${
                    ...
                    <button
                      type="button"
                      onClick={handleFillCurrentArtwork}
                      className="px-2.5 py-1 border border-[#c9a84c]/50 bg-[#c9a84c]/15 text-[#c9a84c] text-[10px] font-mono uppercase hover:bg-[#c9a84c]/25 cursor-pointer font-bold"
                    >
                      Remplir A4 ({perPage} ex. œuvre active)
                    </button>
                    <button
                      type="button"
                      onClick={handleLoadSampleExhibition}
                      className={`px-2.5 py-1 border text-[10px] font-mono uppercase cursor-pointer ${
                        isDark ? "border-neutral-700 hover:bg-white/10 text-stone-300" : "border-stone-300 hover:bg-stone-200 text-stone-700"
                      }`}
                    >
                      + Charger 8 œuvres variées (Photos & Peintures)
                    </button>
                    <button
                      type="button"
                      onClick={() => setIsAddingArtwork(!isAddingArtwork)}
                      className="px-2.5 py-1 bg-[#c9a84c] hover:bg-[#d8b85c] text-black text-[10px] font-mono uppercase font-bold flex items-center gap-1 cursor-pointer ml-auto"
                    >
                      <Plus className="w-3 h-3" /> Ajouter une Œuvre
                    </button>
                  </div>

                  {/* Add Artwork Inline Form */}
                  {isAddingArtwork && (
                    <form onSubmit={handleAddNewArtwork} className={`p-3.5 border space-y-3 ${isDark ? "bg-neutral-900 border-neutral-700" : "bg-stone-100 border-stone-300"}`}>
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-[#c9a84c] font-sans">
                          Ajouter une photographie ou peinture à l'exposition
                        </span>
                        <button 
                          type="button" 
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                        <div>
                          <label className="block text-[10px] font-mono text-stone-400 mb-1">Titre *</label>
                          <input
                            type="text"
                            value={newArtTitle}
                            ...
                            required
                            className={`w-full p-1.5 text-xs border ${isDark ? "bg-black border-neutral-700 text-white" : "bg-white border-stone-300"}`}
                          />
                        </div>
                        <div>
                          <label className="block text-[10px] font-mono text-stone-400 mb-1">Artiste</label>
                          <input
                            type="text"
                            value={newArtArtist}
                            onChange={(e) => setNewArtArtist(e.target.value)}
                            placeholder="Nom de l'artiste"
                            className={`w-full p-1.5 text-xs border ${isDark ? "bg-black border-neutral-700 text-white" : "bg-white border-stone-300"}`}
                          />
                        </div>
                        <div>
                          <label className="block text-[10px] font-mono text-stone-400 mb-1">Médium / Technique</label>
                          <input
                            type="text"
                            value={newArtMedium}
                            onChange={(e) => setNewArtMedium(e.target.value)}
                            placeholder="Ex: Tirage argentique baryté"
                            className={`w-full p-1.5 text-xs border ${isDark ? "bg-black border-neutral-700 text-white" : "bg-white border-stone-300"}`}
                          />
                        </div>
                        <div>
                          <label className="block text-[10px] font-mono text-stone-400 mb-1">Dimensions</label>
                          <input
                            type="text"
                            value={newArtDimensions}
                            onChange={(e) => setNewArtDimensions(e.target.value)}
                            placeholder="Ex: 50 × 75 cm"
                            className={`w-full p-1.5 text-xs border ${isDark ? "bg-black border-neutral-700 text-white" : "bg-white border-stone-300"}`}
                          />
                        </div>
                        <div>
                          <label className="block text-[10px] font-mono text-stone-400 mb-1">Prix ou Statut</label>
                          <input
                            type="text"
                            value={newArtPrice}
                        ...
                        <div>
                          <label className="block text-[10px] font-mono text-stone-400 mb-1">Type</label>
                          <select
                            value={newArtType}
                          ...
                          >
                            ...
                            <option value="dessin">Dessin / Estampe</option>
                      ...
                      <div className="flex justify-end gap-2 pt-1">
                        <button
                          type="button"
                          onClick={() => setIsAddingArtwork(false)}
                          className="px-3 py-1 text-xs border border-neutral-600 text-stone-300 cursor-pointer"
                        >
                          Annuler
                        </button>
                        <button
                          type="submit"
                          className="px-4 py-1 bg-[#c9a84c] text-black font-bold text-xs uppercase cursor-pointer"
                        >
                  ...
                  <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
                    {exhibitionArtworks.map((art) => {
                      const isPhoto = art.type === "photo" || (art.medium || "").toLowerCase().includes("photo");
                      const isSculpture = art.type === "sculpture";
                      return (
                        <div
                          key={art.id}
                          className={`p-2.5 border flex items-center justify-between gap-3 transition-colors ${
                            art.enabled
                              ? isDark ? "bg-neutral-900/60 border-neutral-700" : "bg-white border-stone-300"
                              : "opacity-40 bg-neutral-900/20 border-neutral-800"
                          }`}
                        >
                          <div className="flex items-center gap-2.5 min-w-0 flex-1">
                            <input
                              ...
                              checked={art.enabled}
                              onChange={() => {
                                setExhibitionArtworks(prev => prev.map(a => a.id === art.id ? { ...a, enabled: !a.enabled } : a));
                              }}
                              className="accent-[#c9a84c] w-4 h-4 cursor-pointer"
                            />
                            <span className={`text-[9px] font-mono uppercase px-1.5 py-0.5 shrink-0 ${
                              isPhoto 
                              ...
                              {isPhoto ? "Photo" : isSculpture ? "Sculpture" : "Peinture"}
                            </span>
                            <div className="min-w-0">
                              ...
                              <p className="text-[10px] text-stone-400 truncate">
                                {art.artist} • <span className="italic">{art.medium}</span> ({art.year}) • {art.dimensions}
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-3 shrink-0">
                            ...
                            {/* Copies Stepper */}
                            <div className="flex items-center border border-neutral-700 bg-black/40">
                              <button
                                type="button"
                                onClick={() => {
                                  setExhibitionArtworks(prev => prev.map(a => a.id === art.id ? { ...a, copies: Math.max(0, a.copies - 1) } : a));
                                }}
                                className="w-5 h-5 flex items-center justify-center text-xs text-stone-400 hover:text-white cursor-pointer"
                              >
                                -
                              </button>
                              <span className="w-6 text-center text-xs font-mono font-bold text-[#c9a84c]">
                                {art.copies}
                              </span>
                              <button
                                type="button"
                                onClick={() => {
                                  setExhibitionArtworks(prev => prev.map(a => a.id === art.id ? { ...a, copies: a.copies + 1 } : a));
                                }}
                                className="w-5 h-5 flex items-center justify-center text-xs text-stone-400 hover:text-white cursor-pointer"
                              >
                                +
                            ...
                            {art.id !== "art-active" && (
                              <button
                                type="button"
                                onClick={() => {
                                  setExhibitionArtworks(prev => prev.filter(a => a.id !== art.id));
                                }}
                                className="text-stone-500 hover:text-red-400 cursor-pointer p-1"
                                title="Supprimer de l'exposition"
                              >
              ...
              {/* REALISTIC A4 PAPER CANVAS PREVIEW (ZÉRO CONTOUR DE DÉCOUPE) */}
              <div className="space-y-3 pt-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Eye className="w-4 h-4 text-[#c9a84c]" />
                    <span className="text-xs font-mono uppercase font-bold text-stone-300">
                      Aperçu Fidèle des Feuilles A4 ({totalPages} page{totalPages > 1 ? "s" : ""} • Prêt pour bristol 250-300g)
                    </span>
                  </div>
                  <span className="text-[10px] font-mono text-stone-400">
                    Dimensions réelles : ~5 cm × {densityConfig.heightMm / 10} cm • Zéro liseré
                  </span>
                </div>

                {/* Printable Ref Container holding all A4 pages */}
                <div 
                  ref={printRef} 
                  id="a4-cartels-print-container"
                  className="space-y-8 flex flex-col items-center bg-stone-900/60 p-4 sm:p-8 border border-neutral-800 overflow-x-auto"
                >
                  {pagedCartels.map((sheetItems, pageIndex) => (
                    <div
                      key={`a4-page-${pageIndex}`}
                      className="a4-sheet-preview w-full max-w-[794px] bg-white text-black p-6 sm:p-8 shadow-2xl relative flex flex-col justify-between"
                      style={{
                        minHeight: "1123px",
                        boxSizing: "border-box"
                      }}
                    >
                      {/* Top Discrete Sheet Header */}
                      <div className="flex items-center justify-between pb-2 mb-4 border-b border-stone-200 text-[10px] font-mono uppercase tracking-widest text-stone-500">
                        <span className="font-bold text-stone-700">
                          {galleryName} • {exhibitionTitle}
                        </span>
                        <span>
                          Format Normalisé A4 • Page {pageIndex + 1} / {totalPages}
                        </span>
                      </div>

                      {/* Cartels Grid (STRICTLY NO BORDERS AROUND CARTELS) */}
                      <div 
                        className="grid gap-3 sm:gap-4 flex-1 items-stretch"
                        style={{
                          gridTemplateColumns: "repeat(3, minmax(0, 1fr))",
                          gridTemplateRows: `repeat(${densityConfig.rows}, minmax(0, 1fr))`
                        }}
                      >
                        {sheetItems.map((cartel, cIdx) => {
                          const isAvailable = cartel.status === "available";
                          const statusLabel = isAvailable ? "Disponible" : cartel.status === "reserved" ? "Réservé" : "Collection";
                          const displayPrice = cartel.price && cartel.price !== "Prix sur demande" ? cartel.price : "Prix sur demande";

                          return (
                            <div
                              key={`cartel-${pageIndex}-${cIdx}`}
                              className="cartel-card-preview bg-white p-2 sm:p-2.5 flex items-center justify-between gap-2 overflow-hidden"
                              style={{
                                border: "none",
                                outline: "none",
                                boxShadow: "none"
                            ...
                            >
                              {/* Left: Artwork Typography Info */}
                              <div className="flex-1 min-w-0 flex flex-col justify-center gap-0.5">
                                <span className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-black truncate leading-tight">
                                  {cartel.artist}
                                </span>
                                <span className="font-serif italic font-bold text-xs sm:text-[13px] text-black leading-snug line-clamp-2">
                                  {cartel.title}
                                </span>
                                <div className="text-[9px] text-stone-600 truncate leading-tight">
                                  <span>{cartel.medium}</span>
                                  {cartel.year && <span className="text-stone-500 ml-1">({cartel.year})</span>}
                                </div>
                                {cartel.dimensions && (
                                  <span className="text-[8.5px] font-mono text-stone-500 leading-none">
                                    {cartel.dimensions}
                                  </span>
                                )}
                                <div className="flex items-center gap-2 mt-1 text-[9px] sm:text-[10px] leading-none">
                              ...
                              {/* Right: Vector QR Code */}
                              <div className="shrink-0 flex flex-col items-center justify-center gap-1 w-14 sm:w-16">
                                <div className="cartel-qr-svg w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center">
                                  <QRCodeSVG
                                    value={cartel.qrUrl}
                                    size={56}
                                    level="M"
                                    className="cartel-qr-code w-full h-full block"
                                  />
                                </div>
                                <span className="text-[6.5px] uppercase font-mono tracking-widest text-stone-500 text-center leading-none">
                                  Audioguide
                                </span>
                              </div>
                            </div>
